...
class DoorDetectNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('enable_pipeline_threads', False)
        ...
        self.get_logger().info(
            f"Performance params: model_path={model_path}, "
    ...
    def _set_v4l2_control(self, control_name, value):
        ...
        try:
            ...
            self.get_logger().info(f"[CAMERA_CTRL] {control_name}={value}")
            ...
            self.get_logger().warn(
                f"[CAMERA_CTRL] failed to set {control_name}={value}: {error_text}"
            )
    ...
    def _open_capture(self, video_path):
        ...
        self.get_logger().info(
            f"[CAMERA] requested: {self.camera_width}x{self.camera_height} "
        ...
        self.get_logger().info(
            f"[CAMERA] actual: {actual_width}x{actual_height} "
    ...
    def _record_perf(self, timings):
        ...
        post_timings = getattr(self.model, "last_postprocess_timings", {})
        for key in POSTPROCESS_STAGE_KEYS:
            self.postprocess_perf_samples[key].append(post_timings.get(key, 0.0))
        ...
        total_fps = 1000.0 / avg["total_ms"] if avg["total_ms"] > 0 else 0.0
        node_fps = sum(self.node_fps_list) / len(self.node_fps_list) if self.node_fps_list else 0.0

        avg_parts = [
            format_ms(PERF_LABELS[key], avg[key])
            for key in PERF_KEYS
        ]
        max_parts = [
            format_ms(PERF_LABELS[key], max_values[key])
            for key in PERF_KEYS
        ]
        ...
        self.get_logger().info(
            ...
            + ", ".join(avg_parts)
            + f", total_fps={total_fps:.2f}, node_fps={node_fps:.2f}"
        )
        self.get_logger().info(
        ...
        post_avg = {
            key: sum(values) / len(values)
            for key, values in self.postprocess_perf_samples.items()
        }
        post_max = {
            key: max(values)
            for key, values in self.postprocess_perf_samples.items()
        ...
        self.get_logger().info(
            "[POST_PERF] avg: "
            + ", ".join(format_ms(key, post_avg[key]) for key in POSTPROCESS_STAGE_KEYS)
            + " | max: "
            + ", ".join(format_ms(key, post_max[key]) for key in POSTPROCESS_STAGE_KEYS)
        )
        branch_total = sum(self.preprocess_branch_counts.values())
        if branch_total > 0:
            branch_parts = [
                f"{key}={self.preprocess_branch_counts[key]}({self.preprocess_branch_counts[key] * 100.0 / branch_total:.1f}%)"
                for key in PREPROCESS_STATS_KEYS
            ]
            self.get_logger().info("[OPENCV_BRANCH] " + ", ".join(branch_parts))
        brightness_count = self.preprocess_brightness_stats["count"]
        if brightness_count > 0:
            self.get_logger().info(
                "[OPENCV_BRIGHTNESS] "
                f"mean_brightness={self.preprocess_brightness_stats['mean_sum'] / brightness_count:.2f}, "
                f"p95_brightness={self.preprocess_brightness_stats['p95_sum'] / brightness_count:.2f}, "
                f"dark_ratio={self.preprocess_brightness_stats['dark_ratio_sum'] / brightness_count:.4f}, "
                f"highlight_ratio={self.preprocess_brightness_stats['highlight_ratio_sum'] / brightness_count:.4f}"
            )
        if self.enable_pipeline_threads:
            now = perf_counter()
            elapsed = max(now - self.pipeline_report_start, 1e-6)
            capture_fps = self.pipeline_capture_frames / elapsed
            infer_fps = self.pipeline_infer_frames / elapsed
            publish_fps = self.pipeline_processed_frames / elapsed
            self.get_logger().info(
                f"[PIPELINE] capture_fps={capture_fps:.2f}, "
                f"infer_fps={infer_fps:.2f}, "
                f"publish_fps={publish_fps:.2f}, "
                f"raw_queue_dropped={self.raw_queue_dropped}, "
                f"infer_queue_dropped={self.infer_queue_dropped}, "
                f"end_to_end_ms={avg['end_to_end_ms']:.2f}"
            )
...
class YOLOv8_Detect:
    def __init__(self, opt):
        ...
        self.class_score_thresholds = np.array(
            [
                max(self.RAW_SCORE_THRESHOLD, CLASS_CONF.get(name, DEFAULT_CLASS_CONF))
        ...
        self.ACCIDENT_SCORE_THRESH = max(
            ...
            CLASS_CONF.get("accident", DEFAULT_CLASS_CONF)
        )
